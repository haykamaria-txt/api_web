...
async function createApp() {
  ...
  function professorOnly(req, res, next) {
    if (req.user?.role !== "professor")
      throw new HttpError(403, "Apenas professores podem realizar reservas.");
    next();
  }
  ...
  async function ensureReservationRules({
    labId,
    data,
    inicio,
    termino,
    ...
    if (hasConflict)
      throw new HttpError(409, "Já existe reserva para este laboratório no horário informado.");
  }
    ...
    asyncHandler(async (req, res) => {
      const updates = {
        ...
        passwordHash: req.body.senha
          ? await hashPassword(String(req.body.senha))
  ...
  app.put(
    ...
    asyncHandler(async (req, res) => {
      ...
      const user = await store.updateUser(req.params.id, {
        ...
        matricula:
          req.user.role === "admin" ? (req.body.matricula ?? current.matricula) : current.matricula,
        curso: req.body.curso ?? current.curso,
        role:
          req.user.role === "admin"
            ? normalizeRole(req.body.role || req.body.perfil || current.role)
            : current.role,
        passwordHash: req.body.senha
          ? await hashPassword(String(req.body.senha))
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json({
        laboratorios: labs.filter(
          (lab) => !capacidadeMinima || Number(lab.capacidade) >= capacidadeMinima,
        ),
      });
    }),
  );
    ...
    asyncHandler(async (req, res) => {
      ...
      const next = {
        ...
        termino: req.body.termino
          ? parseTime(req.body.termino, "Horário de término")
          : current.termino,
        quantidadeAlunos:
          req.body.quantidadeAlunos !== undefined
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const labSchedule = schedule.find(
        (item) => Number(item.laboratorio.id) === Number(req.params.id),
      );
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      const reservations = (await store.listReservations({ data })).filter((reservation) =>
        activeReservationStatuses.has(reservation.status),
      );
      res.json({ reservas: await Promise.all(reservations.map(enrichReservation)) });
    }),
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json({
        ...
        porStatus: byStatus,
        problemas: await Promise.all(problems.map(enrichProblem)),
      });
    }),
  );
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      res.json({
        ...
        itensIndisponiveis: inventory.reduce(
          (sum, item) => sum + Number(item.indisponivel || 0),
          0,
        ),
      });
    }),
